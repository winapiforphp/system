...
PHP_METHOD(WinSystemRegistry, getValue)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "ss", &key_full, &key_full_len, &value, &value_len) == FAILURE)
        return;
    ...
    if (RegGetValue(key, key_subkey, value, RRF_RT_ANY | RRF_NOEXPAND, &type, NULL, &size) != ERROR_SUCCESS)
    {
...
PHP_METHOD(WinSystemRegistry, setValue)
{
    ...
    else if (value_type == REG_QWORD || value_type == REG_QWORD_LITTLE_ENDIAN)
    {
        ...
        if (Z_TYPE_P(data_zval) != IS_STRING)
        {
            zend_throw_exception_ex(ce_winsystem_exception, 0 TSRMLS_CC, "expected string for qword type, got %s",
                zend_zval_type_name(data_zval));
            return;
        }
...
PHP_MINIT_FUNCTION(winsystem_registry)
{
    ...
    INIT_NS_CLASS_ENTRY(ce, PHP_WINSYSTEM_NS, "Registry", functions);
